...
namespace Breeze.Persistence.NH {
  ...
  public class NHMetadataBuilder {
    ...
    private List<MetaEnum> _enumList;
    ...
    private MetaDataProperty MakeDataProperty(string propName, IType type, bool isNullable, bool isKey, bool isVersion) {
      ...
      if (type is AbstractEnumType) {
        var types = type.GetType().GetGenericArguments();
        if (types.Length > 0) {
          var realType = types[0];
          dmap.DataType = realType.FullName;
          dmap.EnumType = realType.Name + ":#" + realType.Namespace;
        }
      }
