...
namespace Breeze.ContextProvider.EF6 {
  ...
  public class EFContextProvider<T> : Breeze.ContextProvider.ContextProvider, IEFContextProvider where T : class, new() {
    ...
    private static void UpdateOriginalValues(ObjectStateEntry entry, EntityInfo entityInfo) {
      ...
      var nullProperties = new Dictionary<int, object>();
      var notNullProperties = new List<Tuple<int, string, object>>();

      // check every null value first
      originalValuesMap.ToList().ForEach(kvp => {
        ...
        if (!(originalValue is JObject)) {
          ...
          if (originalValueConverted == null) {
            ...
            nullProperties.Add(ordinal, temp);

            SetDbRecordValue(entry.CurrentValues, ordinal, null);
          }
          else
            notNullProperties.Add(new Tuple<int, string, object>(ordinal, propertyName, originalValueConverted));
        }
      });
      ...
      if (nullProperties.Count > 0) {
        ...
        foreach (var kvp in nullProperties) {
          var ordinal = kvp.Key;
          var temp = kvp.Value;

          SetDbRecordValue(entry.CurrentValues, ordinal, temp);
        }
      }
      ...
      notNullProperties.ForEach(kvp => {
        ...
        entry.SetModifiedProperty(propertyName);
        SetDbRecordValue(originalValuesRecord, ordinal, originalValue);
      });

    }

    private static void SetDbRecordValue(DbUpdatableDataRecord record, int ordinal, object value) {
      try {
        if (value == null)
