...
namespace Breeze.ContextProvider.EF6 {
  ...
  public class EFContextProvider<T> : Breeze.ContextProvider.ContextProviderAsync, IEFContextProvider where T : class, new() {
	...
	/// </summary>
	/// <returns></returns>
	protected override async Task OpenDbConnectionAsync()
	{
		var ec = ObjectContext.Connection as EntityConnection;
		if (ec.State == ConnectionState.Closed) await ec.OpenAsync();
	}
    ...
    public object[] GetKeyValues(object entity) {
	  ...
	  void ProcessSaveWorkState(SaveWorkState saveWorkState)
	{
	  ...
	  void ProcessDbEntityValidationException(SaveWorkState saveWorkState,DbEntityValidationException e)
	  {
	  ...
	  bool ProcessDataException(DataException e)
	  {
	...
	ProcessSaveWorkState(saveWorkState);
      int count;
      try {
        if (Context is DbContext) {
		  ...
		  ProcessDbEntityValidationException(saveWorkState, e);

      } catch (DataException e) {
		  if (!ProcessDataException(e))
			throw;
      } catch (Exception e) {
        throw;
      }
	...
	protected override async Task SaveChangesCoreAsync(SaveWorkState saveWorkState)
	{
		var saveMap = saveWorkState.SaveMap;
		var deletedEntities = ProcessSaves(saveMap);
		...
		try
		{
			if (Context is DbContext)
			{
				count = await ((DbContext)(object)Context).SaveChangesAsync();
			}
			else
			{
				count = await ObjectContext.SaveChangesAsync(System.Data.Entity.Core.Objects.SaveOptions.AcceptAllChangesAfterSave);
			}
		}
		catch (DbEntityValidationException e)
		{
