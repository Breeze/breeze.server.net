...
namespace Breeze.ContextProvider {
  ...
  public abstract class ContextProvider {
    ...
    public Func<Dictionary<Type, List<EntityInfo>>, Task<Dictionary<Type, List<EntityInfo>>>> BeforeSaveEntitiesDelegateAsync { get; set; }
    ...
    protected internal virtual async Task<Dictionary<Type, List<EntityInfo>>> BeforeSaveEntitiesAsync(Dictionary<Type, List<EntityInfo>> saveMap)
    {
        if (BeforeSaveEntitiesDelegateAsync != null)
        {
            return await BeforeSaveEntitiesDelegateAsync(saveMap);
        }
        else
        {
            return saveMap;
  ...
  public class SaveWorkState {
    ...
    {
        SaveMap = BeforeSaveInternal();
        SaveMap = await ContextProvider.BeforeSaveEntitiesAsync(SaveMap);
    }

    Dictionary<Type, List<EntityInfo>> BeforeSaveInternal()
    {
