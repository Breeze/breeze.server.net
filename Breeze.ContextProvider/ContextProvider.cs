...
namespace Breeze.ContextProvider {
  ...
  public abstract class ContextProvider {
    ...
    public Func<Dictionary<Type, List<EntityInfo>>, List<EntityKey>, Dictionary<Type, List<EntityInfo>>> BeforeSaveEntitiesDelegate2 { get; set; }
    public Action<Dictionary<Type, List<EntityInfo>>, List<KeyMapping>> AfterSaveEntitiesDelegate { get; set; }
    public Action<Dictionary<Type, List<EntityInfo>>, List<KeyMapping>, List<EntityKey>> AfterSaveEntitiesDelegate2 { get; set; }
    ...
    protected internal virtual Dictionary<Type, List<EntityInfo>> BeforeSaveEntities(Dictionary<Type, List<EntityInfo>> saveMap, List<EntityKey> deletes = null) {
      if (BeforeSaveEntitiesDelegate != null) {
        saveMap = BeforeSaveEntitiesDelegate(saveMap);
      } if (BeforeSaveEntitiesDelegate2 != null) {
        saveMap = BeforeSaveEntitiesDelegate2(saveMap, deletes);
      } 
      return saveMap;
    }

    /// <summary>
    /// Kept for backward compatibility.  Calls the 2-parameter version of this method.
    /// </summary>
    ...
    protected internal virtual Dictionary<Type, List<EntityInfo>> BeforeSaveEntities(Dictionary<Type, List<EntityInfo>> saveMap) {
      return BeforeSaveEntities(saveMap, null);
    }
    ...
    /// <param name="deletes">The keys for entities that are deleted on the server and should be removed on the client</param>
    protected internal virtual void AfterSaveEntities(Dictionary<Type, List<EntityInfo>> saveMap, List<KeyMapping> keyMappings, List<EntityKey> deletes = null) {
      ...
      if (AfterSaveEntitiesDelegate2 != null) {
        AfterSaveEntitiesDelegate2(saveMap, keyMappings, deletes);
      }
    }

    /// <summary>
    /// Kept for backward compatibility.  Calls the 3-parameter version of this method.
    /// </summary>
    /// <param name="saveMap"></param>
    /// <param name="keyMappings"></param>
    protected internal virtual void AfterSaveEntities(Dictionary<Type, List<EntityInfo>> saveMap, List<KeyMapping> keyMappings) {
      this.AfterSaveEntities(saveMap, keyMappings, null);
    }
  ...
  public class SaveWorkState {
    ...
    public void BeforeSave() {
      ...
      SaveMap = ContextProvider.BeforeSaveEntities(SaveMap, Deletes);
    ...
    public void AfterSave() {
      ContextProvider.AfterSaveEntities(SaveMap, KeyMappings, Deletes);
    }
